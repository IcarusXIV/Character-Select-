...
if (isAdvancedModeCharacter)
        ...
        private void DrawCharacterGrid()
        {
            ...
            for (int i = 0; i < plugin.Characters.Count; i++)
            {
                ...
                {
                    var filteredChar = plugin.Characters[i];
    ...
    : character.ImagePath,
        ...
        private void DrawNameplate(Character character, float width, float height)
        {
            ...
            // 📄 RP Profile Button (Right Side)
            string profileIcon = "RP";
            var profileTextSize = ImGui.CalcTextSize(profileIcon);
            var profilePos = new Vector2(cursorPos.X + width - profileTextSize.X - 15, cursorPos.Y + 6);

            // 🖼️ Draw the icon
            drawList.AddText(profilePos, ImGui.GetColorU32(ImGuiCol.Text), profileIcon);

            // 🖱️ Clickable area (slightly larger than icon for easier clicking)
            var iconClickSize = profileTextSize + new Vector2(6, 6); // Slight padding
            var clickRectMin = profilePos;
            var clickRectMax = profilePos + iconClickSize;

            if (ImGui.IsMouseHoveringRect(clickRectMin, clickRectMax) && ImGui.IsMouseClicked(ImGuiMouseButton.Left))
            {
