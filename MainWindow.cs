...
namespace CharacterSelectPlugin.Windows
{
    public class MainWindow : Window, IDisposable
    {
        ...
        private string editedCharacterFolder = "";
        private bool showFolderFilter = false;
    ...
    : base("Character Select+", ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse)
        ...
        public override void Draw()
        {
            ...
            if (!plugin.IsAddCharacterWindowOpen && !isEditCharacterWindowOpen)
            {
                ...
                // 📂 Folder Toggle + Dropdown (like Search)
                float folderDropdownWidth = 200f;
                float folderIconOffset = 70f;
                float folderDropdownOffset = folderDropdownWidth + folderIconOffset + 10;

                ImGui.SameLine(ImGui.GetWindowWidth() - folderIconOffset);
                ...
                // ⬇ Folder Filter Dropdown (only shows if toggled)
                if (showFolderFilter)
                {
                    ImGui.SameLine(ImGui.GetWindowWidth() - folderDropdownOffset);
                    ImGui.SetNextItemWidth(folderDropdownWidth);
                    if (ImGui.BeginCombo("##FolderFilter", selectedFolder))
                    {
                        var allFolders = plugin.Characters
                            .SelectMany(c => c.Folders ?? new List<string>())
                            .Distinct()
                            .OrderBy(f => f)
                            .Prepend("All")
                            .ToList();
        ...
        private void DrawCharacterForm()
        {
            ...
            string tempFolder = isEditCharacterWindowOpen ? editedCharacterFolder : plugin.NewCharacterFolder;
            ...
            // 📁 Folder Tags Input (comma-separated)
            ImGui.SetCursorPosX(10);
            ImGui.Text("Folders (Optional)");
            ImGui.SameLine(labelWidth);
            ImGui.SetCursorPosX(labelWidth + inputOffset);
            ImGui.SetNextItemWidth(inputWidth);
            ImGui.InputTextWithHint("##CharacterFolders", "e.g. Casual, Battle, Beach", ref tempFolder, 100);

            // ⬅ Save depending on Add/Edit mode
            if (isEditCharacterWindowOpen)
                editedCharacterFolder = tempFolder;
            else
                plugin.NewCharacterFolder = tempFolder;
            ...
            {
                ImGui.BeginTooltip();
                ImGui.TextUnformatted("You can assign multiple folders by separating them with commas.");
...
if (isAdvancedModeCharacter)
    ...
    .Where(c => selectedFolder == "All" || (c.Folders?.Contains(selectedFolder) ?? false))
    .ToList();
...
;
    ...
    : ExtractGlamourerDesignFromMacro(design.Macro);
        ...
        private void OpenEditCharacterWindow(int index)
        {
            ...
            editedCharacterFolder = character.Folders != null && character.Folders.Count > 0
    ? string.Join(", ", character.Folders)
    : "";
        ...
        private void SaveEditedCharacter()
        {
            ...
            character.Folders = string.IsNullOrWhiteSpace(editedCharacterFolder)
    ? new List<string>()
    : editedCharacterFolder.Split(',').Select(f => f.Trim()).ToList();
