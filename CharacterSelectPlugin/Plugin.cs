...
namespace CharacterSelectPlugin
{
    public sealed class Plugin : IDalamudPlugin
    {
        ...
        /// <summary>Patch notes content version.</summary>
        public const string PatchNotesVersion = "2.1.2";
        ...
        public unsafe Plugin(IGameInteropProvider gameInteropProvider)
        {
            ...
            // ASCII-only range (space..tilde) for Oswald fonts. Drops glyph count
            // from ~700 to 95, much cheaper atlas. Outfit keeps extended range so
            // user-supplied character names + RP content still render.
            ushort[] asciiOnlyRange = { 0x0020, 0x007E, 0 };

            // Symbol + extended-Latin fallback range merged onto content fonts
            // (Outfit) so user-typed ♥, ★, →, é, etc. render instead of '?'.
            // Latin-1 (0x20-0xFF) is already covered by the primary Outfit
            // face, so this range only needs to fill what Outfit lacks.
            ushort[] symbolsFallbackRange =
            {
                0x0100, 0x017F, // Latin Extended-A (Polish, Czech, Hungarian)
                0x0180, 0x024F, // Latin Extended-B
                0x2000, 0x206F, // General Punctuation (em dash, smart quotes, ellipsis)
            ...
            // Regular (400) - variable font default
            ...
            // Outfit fonts keep the default range, they render user-supplied
            // content (character aliases, design names, RP profile bios) which
            // can contain any language.
            ...
            // SD Glitch (full licensed version) for the per-character glitch name effect.
            // Two bake sizes: small for character cards, large for the RP profile name header.
            ...
            // Hero-stats value fonts - big numbers in CORE / BONUS / POINTS row.
            // HTML spec: 28/22/20/16px Oswald 600 - use SemiBold static face.
            // ASCII-only since these only ever render digits + a few symbols.
            ...
            // Hero display for shop vault - dedicated dramatic sizes. ASCII
            // only because only ever renders "VAULT SEALED" / percentages.
            ...
            // Preload all custom font handles in parallel so first-frame Push()
            // calls don't fall through to the default face during atlas builds
        ...
        private void DrawUI()
        {
            ...
            // Trigger an async font atlas rebuild if the user's UI scale (or
            // the plugin's UIScaleMultiplier) has changed since the last bake.
            // Cheap each frame; the actual rebuild only fires after a debounce.
            ScaleChangeWatcher();

            // Skip window rendering until the custom font atlas has finished
            // building. Otherwise IFontHandle.Push() is a no-op and the UI lays
            // itself out against the default ImGui font (tiny, wrong metrics),
            // then visibly snaps when the real fonts arrive a moment later.
            if (!FontsReady)
                return;

            // Plugin-wide tooltip styling. Pushed at the top so every tooltip
            // raised inside any window inherits the same look: no rounded
            // corners, 1px boutique border, dark velvet bg, gold-tinted edge.
            // Without this, individual windows pushed their own WindowRounding
            // (often 10) and tooltips inherited a chunky rounded look that
            // didn't match the boutique chassis.
