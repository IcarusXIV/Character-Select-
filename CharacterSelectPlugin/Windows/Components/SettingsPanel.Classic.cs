...
namespace CharacterSelectPlugin.Windows.Components
{
    public partial class SettingsPanel
    {
        ...
        private bool renameMigrationSettingsOpen = false;
        ...
        public void DrawClassicLayout()
{
            ...
            if (ImGui.Begin("Character Select+ Settings", ref isSettingsOpen, windowFlags))
            {
                ...
                ClassicApplyFixedStyles(totalScale);

                try
                {
                    DrawClassicFixedSettingsContent();
                }
                finally
                {
                    ImGui.PopStyleVar(4);
        ...
        private void ClassicApplyFixedStyles(float totalScale)
{
            // Styling
        ...
        private void DrawClassicFixedSettingsContent()
{
            var totalScale = GetSafeScale(ImGuiHelpers.GlobalScale * plugin.Configuration.UIScaleMultiplier);
            ...
            if (ImGui.BeginChild("AllSettings", new Vector2(0, 0), false))
            {
                ...
                visualSettingsOpen = DrawClassicModernCollapsingHeader("Visual Settings", new Vector4(1.0f, 0.35f, 0.35f, 1.0f), visualSettingsOpen, FeatureKeys.CustomTheme);
                if (visualSettingsOpen)
                {
                    DrawClassicVisualSettings(labelWidth, inputWidth);
                }

                // Glamourer Automations Section (Orange)
                automationSettingsOpen = DrawClassicModernCollapsingHeader("Glamourer Automations", new Vector4(1.0f, 0.6f, 0.2f, 1.0f), automationSettingsOpen);
                if (automationSettingsOpen)
                {
                    DrawClassicAutomationSettings();
                }

                // Behavior Settings Section (Yellow)
                behaviorSettingsOpen = DrawClassicModernCollapsingHeader("Behavior Settings", new Vector4(1.0f, 0.9f, 0.3f, 1.0f), behaviorSettingsOpen);
                if (behaviorSettingsOpen)
                {
                    DrawClassicBehaviorSettings();
                }

                // Achievements Section (Gold)
                achievementSettingsOpen = DrawClassicModernCollapsingHeader("Achievements", new Vector4(1.0f, 0.84f, 0.0f, 1.0f), achievementSettingsOpen);
                if (achievementSettingsOpen)
                {
                    DrawClassicAchievementSettings();
                }

                // Random Groups Section (Yellow-Green)
                randomGroupsSettingsOpen = DrawClassicModernCollapsingHeader("Random Groups", new Vector4(0.85f, 0.95f, 0.3f, 1.0f), randomGroupsSettingsOpen);
                if (randomGroupsSettingsOpen)
                {
                    DrawClassicRandomGroupsSettings();
                }

                // Honorific Section (Lime)
                honorificSettingsOpen = DrawClassicModernCollapsingHeader("Honorific", new Vector4(0.7f, 1.0f, 0.3f, 1.0f), honorificSettingsOpen, FeatureKeys.Honorific);
                if (honorificSettingsOpen)
                {
                    DrawClassicHonorificSettings();
                }

                // Main Character Section (Green)
                mainCharacterSettingsOpen = DrawClassicModernCollapsingHeader("Main Character", new Vector4(0.3f, 0.9f, 0.4f, 1.0f), mainCharacterSettingsOpen);
                if (mainCharacterSettingsOpen)
                {
                    DrawClassicMainCharacterSettings(labelWidth, inputWidth);
                }

                // Character Assignments (Cyan)
                characterAssignmentSettingsOpen = DrawClassicModernCollapsingHeader("Character Assignments", new Vector4(0.3f, 0.9f, 0.9f, 1.0f), characterAssignmentSettingsOpen);
                if (characterAssignmentSettingsOpen)
                {
                    DrawClassicCharacterAssignmentSettings();
                }

                // Job Assignments (Teal)
                jobAssignmentSettingsOpen = DrawClassicModernCollapsingHeader("Job Assignments", new Vector4(0.2f, 0.8f, 0.85f, 1.0f), jobAssignmentSettingsOpen, FeatureKeys.JobAssignments);
                if (jobAssignmentSettingsOpen)
                {
                    DrawClassicJobAssignmentSettings();
                }

                // Immersive Dialogue (Blue)
                dialogueSettingsOpen = DrawClassicModernCollapsingHeader("Immersive Dialogue", new Vector4(0.4f, 0.6f, 1.0f, 1.0f), dialogueSettingsOpen);
                if (dialogueSettingsOpen)
                {
                    DrawClassicDialogueSettings();
                }

                // Name Sync (Indigo)
                nameSyncSettingsOpen = DrawClassicModernCollapsingHeader("Name Sync", new Vector4(0.55f, 0.4f, 1.0f, 1.0f), nameSyncSettingsOpen, FeatureKeys.NameSync);
                if (nameSyncSettingsOpen)
                {
                    DrawClassicNameSyncSettings();
                }

                // Conflict Resolution (Purple/Violet)
                conflictResolutionSettingsOpen = DrawClassicModernCollapsingHeader("Conflict Resolution", new Vector4(0.8f, 0.4f, 1.0f, 1.0f), conflictResolutionSettingsOpen);
                if (conflictResolutionSettingsOpen)
                {
                    DrawClassicConflictResolutionSettings();
                }

                // Backup & Restore (Pink/Magenta)
                backupSettingsOpen = DrawClassicModernCollapsingHeader("Backup & Restore", new Vector4(1.0f, 0.45f, 0.7f, 1.0f), backupSettingsOpen);
                if (backupSettingsOpen)
                {
                    DrawClassicBackupSettings();
                }

                renameMigrationSettingsOpen = DrawClassicModernCollapsingHeader("Account & Data", new Vector4(1.0f, 0.55f, 0.55f, 1.0f), renameMigrationSettingsOpen);
                if (renameMigrationSettingsOpen)
                {
                    DrawClassicAccountAndDataSettings();
                }
            }
            ImGui.EndChild();
        }

        private bool DrawClassicModernCollapsingHeader(string title, Vector4 titleColor, bool currentState)
{
            return DrawClassicModernCollapsingHeader(title, titleColor, currentState, null);
        }

        private bool DrawClassicModernCollapsingHeader(string title, Vector4 titleColor, bool currentState, string? featureKey)
        {
            ...
            ImGui.PushStyleColor(ImGuiCol.Header, new Vector4(titleColor.X * 0.6f, titleColor.Y * 0.6f, titleColor.Z * 0.6f, 0.7f));
            ImGui.PushStyleColor(ImGuiCol.HeaderHovered, new Vector4(titleColor.X * 0.7f, titleColor.Y * 0.7f, titleColor.Z * 0.7f, 0.8f));
            ImGui.PushStyleColor(ImGuiCol.HeaderActive, new Vector4(titleColor.X * 0.8f, titleColor.Y * 0.8f, titleColor.Z * 0.8f, 0.9f));
        ...
        private void DrawClassicAccountAndDataSettings()
{
            ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.8f, 0.9f, 1.0f, 1f));
            ImGui.TextWrapped("If you renamed a character before CS+ gained automatic rename detection, use these options to reclaim likes and RP profile data associated with the old name.");
            ImGui.PopStyleColor();

            ImGui.Spacing();
            ImGui.TextColored(new Vector4(0.6f, 0.7f, 0.9f, 1f), "The migration runs on the next upload of each affected character (i.e. next time you apply it).");
            ImGui.Spacing();
            ImGui.Separator();
            ImGui.Spacing();

            // --- Flow A: I renamed a CS+ character ---
            ImGui.TextColored(new Vector4(1f, 0.9f, 0.5f, 1f), "I renamed a CS+ character");
            ImGui.Spacing();

            var characters = plugin.Characters;
            if (characters.Count == 0)
            {
                ImGui.TextColored(new Vector4(0.7f, 0.7f, 0.7f, 1f), "No CS+ characters to migrate.");
            }
            else
            {
                ...
                string currentPreview = characters[manualMigrationCharacterIndex].Name;
                ImGui.SetNextItemWidth(250);
                if (ImGui.BeginCombo("##MigrateCSCharDropdown", currentPreview))
                {
                    for (int i = 0; i < characters.Count; i++)
                    {
                        bool selected = i == manualMigrationCharacterIndex;
                        if (ImGui.Selectable(characters[i].Name, selected))
                            manualMigrationCharacterIndex = i;
                        if (selected) ImGui.SetItemDefaultFocus();
                    }
                    ImGui.EndCombo();
                }

                ImGui.Text("Was previously named:");
                ImGui.SetNextItemWidth(250);
                ImGui.InputText("##MigratePrevCSName", ref manualMigrationPreviousName, 100);

                ImGui.Spacing();
                bool canApplyCSFlow = !string.IsNullOrWhiteSpace(manualMigrationPreviousName);
                if (!canApplyCSFlow) ImGui.BeginDisabled();
                if (ImGui.Button("Apply CS+ Rename Migration##CS"))
                {
            ...
            // --- Flow B: I renamed my in-game character ---
            ImGui.TextColored(new Vector4(1f, 0.9f, 0.5f, 1f), "I renamed my in-game character");
            ImGui.Spacing();

            ImGui.Text("Previous in-game name:");
            ImGui.SameLine();
            ImGui.TextColored(new Vector4(0.7f, 0.7f, 0.7f, 1f), "(format: Name@Server)");
            ImGui.SetNextItemWidth(250);
            ImGui.InputText("##MigratePrevPhysical", ref manualMigrationPhysicalName, 100);

            ImGui.TextWrapped("Applies to every CS+ character currently linked to your in-game character.");

            ImGui.Spacing();
            bool canApplyPhysFlow = !string.IsNullOrWhiteSpace(manualMigrationPhysicalName) && manualMigrationPhysicalName.Contains('@');
            if (!canApplyPhysFlow) ImGui.BeginDisabled();
            if (ImGui.Button("Apply In-Game Rename Migration##Physical"))
            {
                ...
                && (DateTime.Now - manualMigrationStatusTime).TotalSeconds < 10)
            {
                ...
                ImGui.TextWrapped(manualMigrationStatusMessage);
                ImGui.PopStyleColor();
            }
            ...
            ImGui.TextColored(new Vector4(1f, 0.9f, 0.5f, 1f), "Server Communication");
            ImGui.Spacing();
            bool disableServer = plugin.Configuration.DisableAllServerCommunication;
            if (ImGui.Checkbox("Disable all server communication", ref disableServer))
            {
                plugin.Configuration.DisableAllServerCommunication = disableServer;
                plugin.Configuration.Save();
            }
            ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.7f, 0.7f, 0.8f, 1f));
            ImGui.TextWrapped("When on, CS+ stops uploading profiles, looking up other users, fetching the gallery, and submitting reports. Existing data on the server is unaffected (use the delete button below to wipe it).");
            ImGui.PopStyleColor();
            ...
            ImGui.TextColored(new Vector4(1f, 0.9f, 0.5f, 1f), "My Data on the Server");
            ImGui.Spacing();
            ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.7f, 0.7f, 0.8f, 1f));
            ImGui.TextWrapped("Wipe every CS+ profile this installation has uploaded to the server (RP profile data, image, likes). The server keeps a record of which install owns each profile slot, so this only touches your own data.");
            ImGui.PopStyleColor();
            ImGui.Spacing();

            if (deleteDataInProgress) ImGui.BeginDisabled();
            if (ImGui.Button("Delete my data from server"))
            {
                ImGui.OpenPopup("DeleteMyDataConfirmClassic");
            }
            if (deleteDataInProgress) ImGui.EndDisabled();

            // Confirmation popup
            ImGui.SetNextWindowSize(new Vector2(420, 0));
            if (ImGui.BeginPopupModal("DeleteMyDataConfirmClassic", ImGuiWindowFlags.AlwaysAutoResize))
            {
                ImGui.TextWrapped("This permanently deletes every profile you've uploaded from this installation. The data cannot be recovered from the server.");
                ImGui.Spacing();
                ...
                ImGui.SameLine();
                if (ImGui.Button("Delete my data", new Vector2(160, 0)))
                {
                    ImGui.CloseCurrentPopup();
            ...
            // Delete-my-data status feedback
            if (!string.IsNullOrEmpty(deleteDataStatusMessage)
                && (DateTime.Now - deleteDataStatusTime).TotalSeconds < 15)
            {
                ImGui.Spacing();
                ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.7f, 0.9f, 0.7f, 1f));
                ImGui.TextWrapped(deleteDataStatusMessage);
                ImGui.PopStyleColor();
            }
        }

        private void DrawClassicCharacterAssignmentSettings()
{
            ...
            if (plugin.Configuration.LastUsedCharacterByPlayer.Any())
            {
                ...
                if (ImGui.TreeNode("Manage Known In-Game Characters"))
                {
                    ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.8f, 0.9f, 1.0f, 1f));
                    ImGui.TextWrapped("These are the in-game characters CS+ has seen you log into. Remove any that no longer exist or that you want pruned from the dropdowns above and in the Gallery Main Character picker.");
                    ImGui.PopStyleColor();

                    ImGui.Spacing();

                    // Current player, so we can mark it in the list (allowed to delete but it re-adds on next apply)
                    ...
                    // Layout: match the Character Assignments style above , truncated text on the
                    // left with tooltip, red SmallButton on the right at a fixed position.
                    float removeButtonWidth = ImGui.CalcTextSize("Remove").X + ImGui.GetStyle().FramePadding.X * 2 + 4;
                    float buttonSpacingKnown = ImGui.GetStyle().ItemSpacing.X;
                    float availableWidth = ImGui.GetContentRegionAvail().X;
                    float maxTextWidth = availableWidth - removeButtonWidth - buttonSpacingKnown - 10;

                    foreach (var kvp in plugin.Configuration.LastUsedCharacterByPlayer.OrderBy(k => k.Key))
                    {
                        string physicalKey = kvp.Key;
                        string csMapping = kvp.Value;
                        bool isCurrent = physicalKey == currentPlayerKey;

                        // Build full display text
                        string displayText = physicalKey;
                        if (isCurrent)
                            displayText += " (currently logged in)";
                        if (!string.IsNullOrEmpty(csMapping))
                            displayText += $" → {csMapping}";

                        string fullTooltip = displayText;
                        ...
                        // Draw coloured text , green for the physical name, grey for the mapping
                        var arrowIndex = truncatedText.IndexOf(" → ");
                        if (arrowIndex > 0)
                        {
                            var namePart = truncatedText.Substring(0, arrowIndex);
                            var restPart = truncatedText.Substring(arrowIndex);

                            ImGui.PushStyleColor(ImGuiCol.Text, isCurrent ? new Vector4(0.7f, 1f, 0.7f, 1f) : new Vector4(0.7f, 0.9f, 0.7f, 1f));
                            ImGui.Text(namePart);
                            ImGui.PopStyleColor();
                        ...
                        {
                            ImGui.PushStyleColor(ImGuiCol.Text, isCurrent ? new Vector4(0.7f, 1f, 0.7f, 1f) : new Vector4(0.9f, 0.9f, 0.9f, 1f));
                            ImGui.Text(truncatedText);
                            ImGui.PopStyleColor();
                        }
                        ...
                        // Remove button , same red SmallButton style as Character Assignments above
                        ImGui.SameLine(availableWidth - removeButtonWidth + buttonSpacingKnown);

                        ImGui.PushStyleColor(ImGuiCol.Button, new Vector4(0.8f, 0.3f, 0.3f, 0.6f));
                        ImGui.PushStyleColor(ImGuiCol.ButtonHovered, new Vector4(0.9f, 0.4f, 0.4f, 0.8f));
                        ImGui.PushStyleColor(ImGuiCol.ButtonActive, new Vector4(1.0f, 0.5f, 0.5f, 1.0f));

                        if (ImGui.SmallButton($"Remove##knownChar_{physicalKey}"))
                        {
                            toRemoveKey = physicalKey;
                        }
                        ImGui.PopStyleColor(3);

                        if (ImGui.IsItemHovered())
                            ImGui.SetTooltip($"Remove {physicalKey} from known characters");
                    }
        ...
        private void DrawClassicBackupSettings()
{
        ...
        private void DrawClassicJobAssignmentSettings()
{
            ...
            if (ImGui.Checkbox("Enable Gearset Assignments", ref enableGearsetAssignments))
            {
                ...
                plugin.Configuration.Save();
                if (enableGearsetAssignments) plugin.AchievementTracker?.OnGearsetAssignmentsEnabled();
            }
            ...
            {
                bool showJobIcons = plugin.Configuration.ShowJobIconsOnCards;
                if (ImGui.Checkbox("Show Job Icons On Character Cards", ref showJobIcons))
                {
                    plugin.Configuration.ShowJobIconsOnCards = showJobIcons;
                    plugin.Configuration.Save();
                }
                DrawTooltip("Show icons for job, role, and gearset assignments in the bottom-right corner of character cards.");

                if (showJobIcons)
                {
                    ImGui.Indent();
                    bool monoIcons = plugin.Configuration.JobIconsMonochrome;
                    if (ImGui.Checkbox("Monochrome Job Icons", ref monoIcons))
                    {
                        plugin.Configuration.JobIconsMonochrome = monoIcons;
                        plugin.Configuration.Save();
                    }
                    DrawTooltip("Use dark neutral job icons that blend with the card instead of role-coloured ones.");
                    ImGui.Unindent();
                }
            }
        ...
        private void DrawClassicNameSyncSettings()
{
            ...
            if (ImGui.Checkbox("Show my CS+ name to myself", ref enableNameReplacement))
            {
                ...
                if (enableNameReplacement) plugin.AchievementTracker?.OnNameSyncEnabled();
            ...
            if (plugin.Configuration.EnableNameReplacement)
            {
                ...
                bool skipMatching = plugin.Configuration.SkipNameSyncWhenNamesMatch;
                if (ImGui.Checkbox("Skip when my name already matches", ref skipMatching))
                {
                    plugin.Configuration.SkipNameSyncWhenNamesMatch = skipMatching;
                    plugin.Configuration.Save();
                }
                DrawTooltip("Doesn't sync or replace your name while the applied character's name matches\nyour in-game character's name.");
            ...
            if (ImGui.Checkbox("Allow others to see my CS+ name", ref allowOthers))
            {
                ...
                if (allowOthers) plugin.AchievementTracker?.OnSharedNameEnabled();
                plugin.Configuration.Save();
            ...
            bool appliedTargets = plugin.Configuration.ShowCSNameOnAppliedTargets;
            if (ImGui.Checkbox("Show CS+ name on Applied targets", ref appliedTargets))
            {
                plugin.Configuration.ShowCSNameOnAppliedTargets = appliedTargets;
                if (!appliedTargets) plugin.LocalTargetOverrides.Clear();
                plugin.Configuration.Save();
            }
            DrawTooltip("When Apply to Target is used on an NPC or mannequin, show the applied CS+ character's name above them.\n\nLocal-only - never sent to other players. Clears on zone change.");
        ...
        private void DrawClassicConflictResolutionSettings()
{
            ImGui.Spacing();
            ...
            {
                // Back up Penumbra collection files before enabling CR
                if (enabled && !plugin.Configuration.EnableConflictResolution)
                {
        ...
        private void DrawClassicVisualSettings(float labelWidth, float inputWidth)
{
            DrawFixedSetting("Classic Mode:", labelWidth, inputWidth, () =>
            {
                bool classic = plugin.Configuration.UseClassicLayout || Plugin.FontFallbackThisSession;
                if (ImGui.Checkbox("##UseClassicLayout", ref classic))
                {
                    plugin.Configuration.UseClassicLayout = classic;
                    plugin.SaveConfiguration();
                    plugin.OnLayoutModeChanged();
                    mainWindow.InvalidateLayout();
                }
                DrawTooltip(Plugin.FontFallbackThisSession
            ...
            DrawFixedSetting("UI Scale:", labelWidth, inputWidth, () =>
            {
                ...
                if (ImGui.SliderFloat("##UIScaleMultiplier", ref v, 0.7f, 1.5f, "%.2f"))
                    _pendingUIScale = v;
                if (ImGui.IsItemDeactivatedAfterEdit() && !float.IsNaN(_pendingUIScale))
                {
                ...
                DrawTooltip("Scales the entire CS+ UI up or down. 1.0 is the default. Heads up: text can take a couple of seconds to adjust after changing.");
            });
            ...
            bool reduceMotion = plugin.Configuration.ReduceMotion;
            if (ImGui.Checkbox("Reduce Motion", ref reduceMotion))
            {
                plugin.Configuration.ReduceMotion = reduceMotion;
                plugin.SaveConfiguration();
            }
            DrawTooltip("Stops decorative animations: drifting glows, scan lines, dust, card streaks, and hover sheens.");

            bool wardrobeNamesOnly = plugin.Configuration.WardrobeDesignNameOnly;
            if (ImGui.Checkbox("Wardrobe: Design Names Only", ref wardrobeNamesOnly))
            {
                plugin.Configuration.WardrobeDesignNameOnly = wardrobeNamesOnly;
                plugin.SaveConfiguration();
            }
            DrawTooltip("Wardrobe cards show just the design name, without the character name in front.");

            bool rosterName = plugin.Configuration.RosterShowsCharacterName;
            if (ImGui.Checkbox("Roster Shows Character Name", ref rosterName))
            {
                plugin.Configuration.RosterShowsCharacterName = rosterName;
                plugin.SaveConfiguration();
            }
            DrawTooltip("Character cards, the Icon Bar, and the Wardrobe show the Character Name instead of the Alias.\nName Sync and RP profiles keep using the Alias.");
                ...
                if (ImGui.BeginCombo("##ThemeDropdown", displayName))
                {
                    ...
                    foreach (ThemeSelection theme in Enum.GetValues<ThemeSelection>())
                    {
                        ...
                        if (ImGui.Selectable(themeDisplayName, isSelected))
                        {
                            ...
                            // Achievement: track distinct seasonal themes used
            ...
            ImGui.PopStyleColor();
            ImGui.Spacing();
            DrawClassicFontSettings(GetSafeScale(ImGuiHelpers.GlobalScale * plugin.Configuration.UIScaleMultiplier));

            ImGui.Spacing();
        }

        private void DrawClassicFontSettings(float totalScale)
        {
            bool rebuilding = Plugin.Instance?.FontsRebuilding == true;
            if (rebuilding)
                ImGui.TextColored(new Vector4(1f, 0.85f, 0.4f, 1f), "Rebuilding fonts...");
            else
                ImGui.TextDisabled("Changes apply on selection. The UI blinks briefly while fonts rebuild.");
            ImGui.Spacing();

            float valueCol = 150f * totalScale;
            for (int i = 0; i < FontRoleLabels.Length; i++)
            {
                ImGui.AlignTextToFramePadding();
                ImGui.Text(FontRoleLabels[i]);
                ImGui.SameLine(valueCol);

                string current = FontRoleDisplayName(i);
                ImGui.SetNextItemWidth(240f * totalScale);
                if (ImGui.BeginCombo($"##classicFontFace{i}", current))
                {
                    ImGui.SetNextItemWidth(220f * totalScale);
                    ImGui.InputTextWithHint($"##classicFontFilter{i}", "Search...", ref _classicFontFilters[i], 128);
                    ImGui.Separator();

                    string filter = _classicFontFilters[i].Trim();
                    var options = FontOptions();
                    for (int o = 0; o < options.Count; o++)
                    {
                        var opt = options[o];
                        // "Default" (index 0) always visible
                        if (o > 0 && filter.Length > 0 && !opt.Contains(filter, StringComparison.OrdinalIgnoreCase))
                            continue;
                        bool isSel = opt.Equals(current, StringComparison.OrdinalIgnoreCase);
                        if (ImGui.Selectable(opt, isSel))
                        {
                            ApplyFontSelection(i, opt);
                            _classicFontFilters[i] = "";
                        }
                        if (isSel) ImGui.SetItemDefaultFocus();
                    }
                    ImGui.EndCombo();
                }
                else if (_classicFontFilters[i].Length != 0)
                {
                    _classicFontFilters[i] = "";
                }

                ImGui.SameLine();
                if (ImGui.Button($"Reset##classicFont{i}"))
                    ResetFontRole(i);
                DrawTooltip("Reset to the built-in font and 1.00x size.");

                ImGui.SetCursorPosX(valueCol);
                float sizeMult = GetFontRoleScale(i);
                ImGui.SetNextItemWidth(240f * totalScale);
                if (ImGui.SliderFloat($"Size##classicFontScale{i}", ref sizeMult, 0.5f, 2.0f, "%.2fx"))
                {
                    SetFontRoleScale(i, sizeMult);
                    _fontScaleDirty[i] = true;
                }
                CommitFontScaleIfReleased(i);

                ImGui.Spacing();
            }
        }

        private bool _classicColorSavePending = false;

        private void DrawClassicColorOption(CustomThemeDefinitions.ColorOption option, CustomThemeConfig customTheme, float totalScale)
{
            ...
            if (!string.IsNullOrEmpty(option.Description))
            {
                ...
                if (ImGui.IsItemHovered())
                {
                    ...
                    ImGui.BeginTooltip();
                    ImGui.PushTextWrapPos(300f * totalScale);
        ...
        private void DrawClassicCustomColorOption(CustomThemeDefinitions.CustomColorOption option, CustomThemeConfig customTheme, float totalScale)
{
            // Get current value (override or default)
            Vector4 currentColor;
            bool hasOverride = customTheme.ColorOverrides.TryGetValue(option.Key, out var packedColor) && packedColor.HasValue;
            ...
            bool deadInClassic = CustomThemeDefinitions.IsDeadInClassic(option.Key);
            if (deadInClassic)
                ImGui.BeginDisabled();
            ...
            if (!string.IsNullOrEmpty(option.Description))
            {
                ...
                if (ImGui.IsItemHovered(ImGuiHoveredFlags.AllowWhenDisabled))
                {
                    ...
                    ImGui.BeginTooltip();
                    ImGui.PushTextWrapPos(300f * totalScale);
                    ImGui.TextUnformatted(deadInClassic
                        ? option.Description + "\n\nNot used by Classic Mode. This colour only appears in the redesigned layout."
                        : option.Description);
                    ImGui.PopTextWrapPos();
                    ImGui.EndTooltip();
                }
        ...
        private void DrawClassicPresetManagement(CustomThemeConfig customTheme, float totalScale)
{
            var presets = plugin.Configuration.ThemePresets;
        ...
        private void DrawClassicCustomColorCategory(string category, CustomThemeConfig customTheme, float totalScale)
{
            ...
            if (ImGui.CollapsingHeader($"{category}##CustomColorCategory", isExpanded ? ImGuiTreeNodeFlags.DefaultOpen : ImGuiTreeNodeFlags.None))
            {
                ...
                foreach (var option in CustomThemeDefinitions.GetCustomColorOptionsForCategory(category))
                {
                    ...
                    {
                        ImGui.BeginDisabled();
                        DrawClassicCustomColorOption(option, customTheme, totalScale);
                        ImGui.EndDisabled();
                    }
                    else
                    {
                ...
                if (category == "Atmosphere")
                {
                    ...
                    if (ImGui.IsItemHovered())
                        ImGui.SetTooltip("Strength of all atmosphere layers, including the gold ones. 0 turns them off; 12 matches the hover spotlight.");
                    ...
                    if (ImGui.SliderFloat("##atmoIntensityClassic", ref intensity, 0f, 12f, "%.1fx"))
                ...
                if (category == "Wardrobe")
                {
                    ...
                    ImGui.Separator();
                    ImGui.Spacing();
                    DrawClassicWardrobeBackgroundSettings(customTheme, totalScale);
                }
        ...
        private void DrawClassicHonorificSettings()
{
        ...
        private void DrawClassicMainCharacterSettings(float labelWidth, float inputWidth)
{
                ...
                if (ImGui.BeginCombo("##MainCharacterDropdown", currentMainChar))
                {
                    ...
                    foreach (var character in plugin.Characters)
                    {
                        ...
                        if (ImGui.Selectable(character.Name, isSelected))
                        {
                            ...
                            plugin.Configuration.Save();
                            plugin.AchievementTracker?.OnMainCharacterSet();
                        }
        ...
        private void DrawClassicBehaviorSettings()
{
            bool showUpdateNotif = plugin.Configuration.ShowUpdateNotification;
            if (ImGui.Checkbox("Notify of updates in chat", ref showUpdateNotif))
            {
                plugin.Configuration.ShowUpdateNotification = showUpdateNotif;
                plugin.Configuration.Save();
            }
            DrawTooltip("Shows a chat message when a new CS+ version is available on GitHub.\nChecked every 30 minutes, notifies once per session.");
            ...
            bool iconBarOpen = plugin.IconBarWindow?.IsOpen ?? false;
            if (ImGui.Checkbox("Icon Bar", ref iconBarOpen) && plugin.IconBarWindow != null)
                plugin.IconBarWindow.IsOpen = iconBarOpen;
            DrawTooltip("A separate thin strip of character portraits with its own position.\nLeft click applies the character, right click lists their designs.\nAlso toggled with /selecticons.");

            if (plugin.IconBarWindow?.IsOpen == true)
            {
                ImGui.Indent();

                float iconScale = plugin.Configuration.QuickSwitchIconBarScale;
                ImGui.SetNextItemWidth(140f);
                if (ImGui.SliderFloat("Icon Bar Size", ref iconScale, 0.5f, 3.0f, "%.2fx"))
                    plugin.Configuration.QuickSwitchIconBarScale = iconScale;
                if (ImGui.IsItemDeactivatedAfterEdit())
                    plugin.Configuration.Save();
                DrawTooltip("Scales the icon bar on its own, separate from the other Quick Switch styles.");

                int maxTiles = plugin.Configuration.QuickSwitchIconBarMaxTiles;
                ImGui.SetNextItemWidth(140f);
                if (ImGui.SliderInt("Icon Bar Characters", ref maxTiles, 5, 15))
                    plugin.Configuration.QuickSwitchIconBarMaxTiles = maxTiles;
                if (ImGui.IsItemDeactivatedAfterEdit())
                    plugin.Configuration.Save();
                DrawTooltip("How many character icons the bar shows before it scrolls.");

                bool favFirst = plugin.Configuration.QuickSwitchIconBarFavouritesFirst;
                if (ImGui.Checkbox("Favourites First", ref favFirst))
                {
                    plugin.Configuration.QuickSwitchIconBarFavouritesFirst = favFirst;
                    plugin.Configuration.Save();
                }
                DrawTooltip("Shows favourited characters at the start of the bar.");

                var orientLabels = new[] { "Auto", "Horizontal", "Vertical" };
                int orientIdx = Math.Clamp(plugin.Configuration.QuickSwitchIconBarOrientation, 0, 2);
                ImGui.SetNextItemWidth(140f);
                if (ImGui.Combo("Orientation", ref orientIdx, orientLabels, orientLabels.Length))
                {
                    plugin.Configuration.QuickSwitchIconBarOrientation = orientIdx;
                    plugin.Configuration.Save();
                }
                DrawTooltip("Auto turns the bar vertical when it sits near the left or right screen edge.\nPick Horizontal or Vertical to lock it.");
            ...
            bool quickSwitchShowSearch = plugin.Configuration.QuickSwitchShowSearch;
            if (ImGui.Checkbox("Quick Switch dropdown search", ref quickSwitchShowSearch))
            {
                plugin.Configuration.QuickSwitchShowSearch = quickSwitchShowSearch;
                plugin.Configuration.Save();
            }
            DrawTooltip("Shows a search field in the Quick Switch character and design dropdowns.");
        ...
        private void DrawClassicDialogueSettings()
{
            // Warning
            ...
            // Requirements
            ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(1.0f, 0.85f, 0.4f, 1f));
            ImGui.TextWrapped("RP Profile required: Each character needs Pronouns set in their RP Profile for this to apply. Characters without pronouns are skipped and the game's default text is shown unchanged.");
            ImGui.PopStyleColor();
            ...
            if (ImGui.Checkbox("Enable Immersive Dialogue", ref enableDialogue))
            {
                ...
                plugin.Configuration.Save();
                if (enableDialogue) plugin.AchievementTracker?.OnImmersiveDialogueEnabled();
            }
        ...
        private void DrawClassicColorCategory(string category, CustomThemeConfig customTheme, float totalScale)
{
            // Initialize category expansion state if needed
            if (!_colorCategoryExpanded.ContainsKey(category))
            {
            ...
            if (ImGui.CollapsingHeader($"{category}##ColorCategory", isExpanded ? ImGuiTreeNodeFlags.DefaultOpen : ImGuiTreeNodeFlags.None))
            {
                ...
                foreach (var option in CustomThemeDefinitions.GetColorOptionsForCategory(category))
                {
                    DrawClassicColorOption(option, customTheme, totalScale);
                }
        ...
        private void DrawClassicWardrobeBackgroundSettings(CustomThemeConfig customTheme, float totalScale)
{
            ...
            ImGui.PushStyleColor(ImGuiCol.Header, new Vector4(0.83f * 0.15f, 0.69f * 0.15f, 0.22f * 0.15f, 0.80f));
            ImGui.PushStyleColor(ImGuiCol.HeaderHovered, new Vector4(0.83f * 0.20f, 0.69f * 0.20f, 0.22f * 0.20f, 0.90f));
            ImGui.PushStyleColor(ImGuiCol.HeaderActive, new Vector4(0.83f * 0.25f, 0.69f * 0.25f, 0.22f * 0.25f, 1.00f));
            bool open = ImGui.CollapsingHeader("Wardrobe Background Image");
            ImGui.PopStyleColor(3);
            ...
            if (currentPath.Length > 40) currentPath = "..." + currentPath.Substring(currentPath.Length - 37);
            ImGui.Text($"Current: {currentPath}");

            if (ImGui.Button("Browse...##wardBg", new Vector2(100f * totalScale, 0)))
            {
            ...
            DrawTooltip("Select an image file to use as the Wardrobe window background.");
            ...
            if (!string.IsNullOrEmpty(customTheme.WardrobeBackgroundImagePath))
            {
                ...
                ImGui.PushStyleColor(ImGuiCol.ButtonHovered, new Vector4(0.7f, 0.4f, 0.4f, 0.8f));
                ImGui.PushStyleColor(ImGuiCol.ButtonActive, new Vector4(0.8f, 0.5f, 0.5f, 1.0f));
                if (ImGui.Button("Clear##wardBgClear", new Vector2(60f * totalScale, 0)))
                {
            ...
            if (!string.IsNullOrEmpty(customTheme.WardrobeBackgroundImagePath))
            {
                ...
                ImGui.Spacing();
                if (ImGui.Button("Reset Position & Zoom##wardBg", new Vector2(150f * totalScale, 0)))
                {
        ...
        private void DrawClassicRandomGroupsSettings()
{
            var totalScale = GetSafeScale(ImGuiHelpers.GlobalScale * plugin.Configuration.UIScaleMultiplier);
        ...
        private void DrawClassicCompactQuickSwitchSettings(CustomThemeConfig customTheme, float totalScale)
{
            ...
            if (ImGui.CollapsingHeader($"{categoryKey}##CompactQSCategory", isExpanded ? ImGuiTreeNodeFlags.DefaultOpen : ImGuiTreeNodeFlags.None))
            {
                ...
                ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.6f, 0.6f, 0.6f, 1f));
                ImGui.TextWrapped("Only affects the compact Quick Switch bar.");
                ImGui.PopStyleColor();
                ImGui.Spacing();
        ...
        private void DrawClassicAchievementSettings()
{
            // Master toggle
            bool enableAchievements = plugin.Configuration.EnableAchievementSystem;
            if (ImGui.Checkbox("Enable Achievement system", ref enableAchievements))
            {
                plugin.Configuration.EnableAchievementSystem = enableAchievements;
                plugin.Configuration.Save();
            }
            DrawTooltip("Master toggle for the entire achievement system. When off, the trophy button, notifications, and chat messages are hidden.\nYour progress is preserved, so re-enabling restores everything as it was.\nFor users who'd rather have CS+ feel like a relaxed sandbox without unlock prompts.");

            // Sub-options greyed out when the master is off
            bool achGated = !enableAchievements;
            if (achGated) ImGui.BeginDisabled();

            ImGui.Indent(20f);

            // Toast notification toggle
            bool showAchNotif = plugin.Configuration.ShowAchievementNotifications;
            if (ImGui.Checkbox("Show achievement toast notifications", ref showAchNotif))
            {
                plugin.Configuration.ShowAchievementNotifications = showAchNotif;
                plugin.Configuration.Save();
            }
            DrawTooltip("Shows a CS+ slide-in toast when you unlock an achievement.\nClick a toast to dismiss it early; up to 3 stack at once.");

            // Position dropdown + Test button (indented under the toast toggle so they associate)
            ImGui.Indent(20f);

            bool toastGated = achGated || !showAchNotif;
            if (!achGated && toastGated) ImGui.BeginDisabled();

            ImGui.AlignTextToFramePadding();
            ImGui.Text("Position:");
            ImGui.SameLine();

            int positionIndex = (int)plugin.Configuration.AchievementToastPosition;
            ImGui.SetNextItemWidth(140f * ImGuiHelpers.GlobalScale);
            if (ImGui.Combo("##achToastPos", ref positionIndex, ToastPositionLabels, ToastPositionLabels.Length))
            {
                plugin.Configuration.AchievementToastPosition = (Configuration.ToastPosition)positionIndex;
                plugin.Configuration.Save();
            }
            DrawTooltip("Where the achievement toast appears on screen.\nPick whichever corner doesn't conflict with your in-game UI layout.");

            ImGui.SameLine();
            if (ImGui.Button("Test##achToastTest"))
            {
                plugin.RunAchievementToastTest();
            }
            DrawTooltip("Queues a few sample toasts so you can preview the chosen position.");

            if (!achGated && toastGated) ImGui.EndDisabled();

            ImGui.Unindent(20f);

            // Chat toggle
            bool showAchChat = plugin.Configuration.ShowAchievementChatMessages;
            if (ImGui.Checkbox("Show achievement messages in chat", ref showAchChat))
            {
                plugin.Configuration.ShowAchievementChatMessages = showAchChat;
                plugin.Configuration.Save();
            }
            DrawTooltip("Prints a chat line when you unlock an achievement.");
        ...
        private void DrawClassicFavoriteIconPicker(CustomThemeConfig customTheme, float totalScale)
{
        ...
        private void DrawClassicCustomThemeEditor()
{
            var totalScale = GetSafeScale(ImGuiHelpers.GlobalScale * plugin.Configuration.UIScaleMultiplier);
            var customTheme = plugin.Configuration.CustomTheme;
            ...
            DrawClassicPresetManagement(customTheme, totalScale);
            ...
            DrawClassicBackgroundImagePicker(customTheme, totalScale);
            ...
            DrawClassicFavoriteIconPicker(customTheme, totalScale);
            ...
            DrawTooltip("The interface follows whoever is currently applied: the accent and everything derived from it, the wardrobe accent, card glows, and the compact Quick Switch all take the active character's nameplate colour.");
            ImGui.Spacing();

            // Accent first
            DrawClassicCustomColorCategory("Accent", customTheme, totalScale);

            foreach (var category in CustomThemeDefinitions.GetColorCategories())
            {
                DrawClassicColorCategory(category, customTheme, totalScale);
            }

            // Custom-only categories not already drawn above
            ...
            foreach (var category in CustomThemeDefinitions.GetCustomColorCategories())
            {
                ...
                {
                    DrawClassicCustomColorCategory(category, customTheme, totalScale);
                }
            }

            // Draw Compact Quick Switch settings (as collapsible category)
            DrawClassicCompactQuickSwitchSettings(customTheme, totalScale);

            // Deferred colour save, flushed on mouse release
            if (_classicColorSavePending && !ImGui.IsAnyMouseDown())
            {
                plugin.Configuration.Save();
        ...
        private void DrawClassicAutomationSettings()
{
            // Warning
            ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(1f, 0.7f, 0.3f, 1f));
        ...
        private void DrawClassicBackgroundImagePicker(CustomThemeConfig customTheme, float totalScale)
{
