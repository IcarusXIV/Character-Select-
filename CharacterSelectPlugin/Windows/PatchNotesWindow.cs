...
namespace CharacterSelectPlugin.Windows
{
    public class PatchNotesWindow : Window
    {
        ...
        // Off for this release; flip to true once we have showcase-worthy cards again.
        ...
        public override void Draw()
        {
            ...
            if (IsOpen && !wasOpen)
            {
                hasScrolledToEnd = !Plugin.PatchNotesRequiresAck;
                hasAcknowledgedNSFW = !Plugin.PatchNotesRequiresAck;
            }
            wasOpen = IsOpen;
        ...
        private void DrawModernHeader(float totalScale)
        {
            ...
            var fallbackHeaderHeight = 180f * totalScale;

            var headerStart = windowPos + new Vector2(windowPadding.X, windowPadding.Y * 1.1f);

            // Resolve banner aspect up front so the "New in vX" line can sit flush below the image.
            float bannerHeight = fallbackHeaderHeight;
            ...
            try
            {
                ...
                if (File.Exists(imagePath))
                {
                    ...
                    if (bannerTex != null)
                    {
                        var imageAspect = (float)bannerTex.Width / bannerTex.Height;
                        bannerHeight = headerWidth / imageAspect;
                    }
                }
            }
            catch (Exception ex)
            ...
            var headerEnd = headerStart + new Vector2(headerWidth, bannerHeight);
            var drawList = ImGui.GetWindowDrawList();

            if (bannerTex != null)
            {
                drawList.AddImage((ImTextureID)bannerTex.Handle, headerStart, headerEnd);
        ...
        private void DrawClassicModeAnnouncementBox(float totalScale)
        {
            ImGui.BeginChild("ClassicModeAnnouncement", new Vector2(0, 215 * totalScale), true, ImGuiWindowFlags.None);

            var bodyColor = new Vector4(0.9f, 0.85f, 0.95f, 1.0f);
            ImGui.PushTextWrapPos();

            ImGui.TextColored(bodyColor,
                "Hi, I wanted to acknowledge the feedback I've gotten about the UI redesign. A lot of you have been understandably unhappy with it, from it feeling too cluttered, too \"gacha\", or \"just ugly\", to how overstimulating it can be with so much going on. My intent with the redesign was to create a new base to allow for more personal customizations; my mistake was simply going off of what I, personally, liked.");

            ImGui.Spacing();

            ImGui.TextColored(bodyColor,
                "This update adds a toggle to return to the original look of the plugin before the redesign (Achievements and Wardrobe aren't included as they did not have prior designs to go back to). You can find it in Settings > Visual > \"Classic Mode\".");

            ImGui.Spacing();

            ImGui.TextColored(bodyColor,
                "This was the biggest priority for me to get out and address the feedback. Going forward, the plan is to introduce ways to customize the redesign to one's preferences: reducing the motion and animations, better controls for the UI colours, and the ability to change fonts and their size.");
        ...
        {
            // Achievements
            DrawFeatureSection("", "Achievements", new Vector4(1.0f, 0.84f, 0.0f, 1.0f));
            ImGui.BulletText("80+ achievements across 8 categories");
            ImGui.BulletText("Trophy button on the main window glows gold for new completions");
            ImGui.BulletText("Slide-in toasts stack up to 3, one after another");
            ImGui.BulletText("Click any toast to dismiss it early");
            ImGui.BulletText("6 positions: corners or top/bottom centre");
            ImGui.BulletText("Retroactive credit for character/design counts, saved profile fields, and more");
            ImGui.BulletText("Built to encourage exploration, not pressure completionists");
            ImGui.BulletText("Points system: save them up for an upcoming rewards shop");
            ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.7f, 0.7f, 0.75f, 1.0f));
            ImGui.TextWrapped("Not for you? Settings > Achievements has a master toggle to hide it all. Progress is preserved if you change your mind.");
            ImGui.PopStyleColor();
            ImGui.Spacing();

            // Wardrobe
            DrawFeatureSection("", "Wardrobe", new Vector4(0.16f, 0.71f, 0.96f, 1.0f));
            ImGui.BulletText("Boutique coverflow: your designs sit on a lit stage; drag, flick, scroll, or arrow-key through them");
            ImGui.BulletText("Editorial info panel shows the focused design's name, mods, last applied, and edition");
            ImGui.BulletText("Click the focus card to apply; click any side card to bring it forward");
            ImGui.BulletText("Right-click the focus card to set its preview from your clipboard or toggle favourite");
            ImGui.BulletText("Open with /wardrobe, the hanger button in the Design Panel, or Shift+Click a Designs button");
            ImGui.BulletText("Per-character accent: optionally use the character's nameplate colour for the chassis");
            ImGui.BulletText("Fully themeable: colours and background image in the Custom Theme editor");
            ImGui.Spacing();

            // Rename Migration
            DrawFeatureSection("", "Rename Migration", new Vector4(0.49f, 0.34f, 0.76f, 1.0f));
            ImGui.BulletText("Your likes carry over when you rename a CS+ character");
            ImGui.BulletText("In-game renames detected automatically on next apply");
            ImGui.BulletText("Manual migration tool in Settings covers renames from before this update");
            ImGui.BulletText("Clean delete: removing a CS+ character now cleans up the server copy too");
            ImGui.Spacing();

            // Privacy Fixes
            DrawFeatureSection("", "Privacy Fixes", new Vector4(1.0f, 0.37f, 0.54f, 1.0f));
            ImGui.BulletText("Private profiles are now properly hidden from /viewrp");
            ImGui.BulletText("Exclude from Name Sync no longer hides your RP profile as well");
            ImGui.BulletText("Shared-name collisions: fixed mixed data when characters shared a display name");
            ImGui.Spacing();

            // QoL & Improvements
            DrawFeatureSection("", "QoL & Improvements", new Vector4(0.95f, 0.17f, 0.49f, 1.0f));
            ImGui.BulletText("Top pagination: page buttons now appear at the top of the character grid");
            ImGui.BulletText("Select All Gear/Hair button in Mod Manager > Currently Affecting You");
            ImGui.BulletText("Quick refresh for Mod Manager on the Add/Edit form");
            ImGui.BulletText("Manage known in-game characters in Settings > Character Assignments");
            ImGui.BulletText("Update notifications now appear in chat when a new CS+ release is available");
            ImGui.BulletText("Fixed Alias and Name Sync exclusion not saving on new characters");
            ImGui.Spacing();

            // Behind the Scenes
            DrawFeatureSection("", "Behind the Scenes", new Vector4(0.42f, 0.48f, 0.56f, 1.0f));
            ImGui.BulletText("Optimised image storage to help keep CS+ running sustainably");
            ImGui.BulletText("Gallery cache: better caching on Gallery requests to reduce server traffic");
            ImGui.BulletText("Name Sync: new CS+ users may take ~60s to first appear nearby");
            ImGui.BulletText("Gallery: currently disabled and under construction");
            ImGui.Spacing();
        }
        ...
        private void Draw110Notes()
        {
            ...
            ImGui.BulletText("May conflict if you are using Glamourer Automations.");
            ImGui.Spacing();
            ...
            ImGui.BulletText("WILL 100 percent conflict if you are using Glamourer Automations.");
            ImGui.Spacing();
            ...
            ImGui.BulletText("Let me know if there are any issues using this.");
            ImGui.BulletText("If your UI is fine as-is, best to leave this be.");
            ImGui.Spacing();
        }
        ...
        private void DrawBottomButton(float totalScale)
        {
            ...
            if (hasScrolledToEnd && Plugin.PatchNotesRequiresAck)
            {
                ImGui.Spacing();

                string checkboxText = "I understand that RP Profiles and CS+ Names may contain mature content";
                float checkboxWidth = ImGui.CalcTextSize(checkboxText).X + 30 * totalScale;
                ImGui.SetCursorPosX((windowWidth - checkboxWidth) * 0.5f);

                ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.9f, 0.75f, 0.5f, 1.0f));
